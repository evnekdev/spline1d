    ...
    T: Float + core::fmt::Debug,
{
    ...
    pub fn interpolate_for_index<Q: ?Sized>(
        &self,
        keyx: &Q,
        keyy: &Q,
        x: &T,
    ...
    pub fn get_break_for_index_by_idx(&self, idx: usize, index: usize) -> Option<T> {
        if index >= self.tt.len() && idx >= self.pps.len() {
            return None;
        }
...
pub fn load_multispline_from_csv(
    datafile: &str,
) -> Result<MultiSpline<String, f64>, Box<dyn Error>> {
    ...
    for result in reader.records() {
        ...
        record
            .into_iter()
            .enumerate()
            .for_each(|(idx, strval)| vecs[idx].push(strval.parse::<f64>().unwrap_or(f64::NAN)));
    }
