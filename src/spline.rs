...
impl<T: Float + fmt::Debug> Spline<T> {
    ...
    pub fn new(xx: &[T], yy: &[T], ss: &[T]) -> Self {
        // xx is the principal variable, yy is a dependent variable
        ...
        let dzzdx: Vec<T> = dxx
            .iter()
            .zip(divdif.iter().zip(ss.iter()))
            .map(|(dx, (dydx, s))| (*dydx - *s) / *dx)
            .collect();
        let dzdxx: Vec<T> = dxx
            .iter()
            .zip(divdif.iter().zip(ss[1..].iter()))
            .map(|(dx, (dydx, s))| (*s - *dydx) / *dx)
            .collect();
        let mut coeffs: Vec<[T; 4]> = (0..xx.len() - 1)
            .map(|idx| {
                [
                    (dzdxx[idx] - dzzdx[idx]) / dxx[idx],
                    (dzzdx[idx] + dzzdx[idx]) - dzdxx[idx],
                    ss[idx],
                    yy[idx],
                ]
            })
    ...
    pub fn interpolate(&self, x: &T) -> Option<T> {
        let index = if self.check_index(&self.last, x) {
            self.last
        } else {
            self.index(x)?
        };
...
impl Spline<f64> {
    ...
    pub fn intersection_with_exact(
        &self,
        other: &Spline<f64>,
        index1: usize,
        index2: usize,
    ...
    pub fn intersection_with_numeric(
        &self,
        other: &Spline<f64>,
        index1: usize,
        index2: usize,
