...
pub fn slopes_makima<T: Float + core::fmt::Debug>(xx: &[T], yy: &[T]) -> Vec<T> {
    ...
    let delta_new: Vec<T> = delta_prefix
        .iter()
        .chain(delta.iter())
        .chain(delta_suffix.iter())
        .map(|&v| v)
        .collect();
    let k1 = [-T::one(), T::one()];
    ...
    let s1: Vec<T> = weights[2..]
        .iter()
        .zip(delta_new[1..n + 1].iter())
        .map(|(w, d)| *w * *d)
        .collect();
    let s2: Vec<T> = weights[0..n]
        .iter()
        .zip(delta_new[2..n + 2].iter())
        .map(|(w, d)| *w * *d)
        .collect();
    let ss: Vec<T> = weights12
        .iter()
...
/// where `dx = x - x1`.
pub fn makima_single_middle<T: Float>(
    x0: T,
    y0: T,
    x1: T,
    y1: T,
...
/// `y = y1*(1-t) + y2*t + (1-t)*t*(alpha0 + alpha1*t)`,
/// where `t = (x - x1) / (x2 - x1)`.
pub fn makima_single_middle_alpha<T: Float>(
    x0: T,
    y0: T,
    x1: T,
    y1: T,
