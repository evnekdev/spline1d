...
/// where `dx = x - x1`.
pub fn akima_single_middle<T: Float>(
    x0: T,
    y0: T,
    x1: T,
    y1: T,
...
/// `y = y1*(1-t) + y2*t + (1-t)*t*(alpha0 + alpha1*t)`,
/// where `t = (x - x1) / (x2 - x1)`.
pub fn akima_single_middle_alpha<T: Float>(
    x0: T,
    y0: T,
    x1: T,
    y1: T,
...
mod tests {
    ...
    fn single_intervals_match_full_slope_coefficients() {
        // Left endpoint: compare against a 3-point full Akima spline
        let xx3 = [0.0, 1.0, 2.0];
        let yy3 = [0.0, 1.0, 1.5];
        let slopes3 = slopes_akima(&xx3, &yy3);

        let left = akima_single_left(xx3[0], yy3[0], xx3[1], yy3[1], xx3[2], yy3[2]);

        let right = akima_single_right(xx3[0], yy3[0], xx3[1], yy3[1], xx3[2], yy3[2]);

        assert_close(left[2], slopes3[0]);
        assert_close(right[2], slopes3[1]);

        // Middle interval: compare against a 4-point full Akima spline
        let xx4 = [0.0, 1.0, 2.0, 4.0];
        let yy4 = [0.0, 1.0, 1.5, 3.0];
        let slopes4 = slopes_akima(&xx4, &yy4);

        let middle = akima_single_middle(
            xx4[0], yy4[0], xx4[1], yy4[1], xx4[2], yy4[2], xx4[3], yy4[3],
        );

        assert_close(middle[2], slopes4[1]);
    }
}
