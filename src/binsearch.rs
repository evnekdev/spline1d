...
pub fn binary_search_interval<T: Float>(
    size: usize,
    sval: &T,
    locator: impl Fn(usize) -> T,
) -> Option<usize> {
    if _check_float(sval) {
        return None;
    }
    let mut n0 = 0;
    let mut n1 = size - 1;
    ...
    let v1 = locator(n1);
    if _check_float(&v0) || _check_float(&v1) {
        return None;
    }
    if !interval_inside(sval, (&v0, &v1)) {
        return None;
    }
    while (n1 - n0) > 1 {
        let n = (n0 + n1) / 2;
