...
use spline1d::{fritschbutland, fritschbutland_single_middle, fritschbutland_single_middle_alpha};
...
fn main() {
    ...
    let pp = fritschbutland(&xx, &yy);

    for x in [0.0, 0.5, 1.0, 1.5, 2.5, 4.0] {
        println!("x = {x}, y = {:?}", pp.interpolate(&x));
    }

    let coeffs = fritschbutland_single_middle(0.0, 0.0, 1.0, 1.0, 2.0, 1.5, 3.0, 1.75);
    let alpha = fritschbutland_single_middle_alpha(0.0, 0.0, 1.0, 1.0, 2.0, 1.5, 3.0, 1.75);
